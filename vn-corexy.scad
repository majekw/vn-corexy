...
module joint_hole(jl=6,screw_l=joint_screw_len,print_upside=false,cut_nut=true){
  union(){
    // hole for thread
    translate([0,0,-screw_l]) cylinder(h=screw_l,d=joint_screw_d+2*printer_off);
    // hole for head
    if (print_upside && bridge_support) {
      translate([0,0,bridge_thickness]) cylinder(h=jl-bridge_thickness,d=joint_screw_washer);
    } else {
      cylinder(h=jl,d=joint_screw_washer);
    }
    // hole for hex tool
    translate([0,0,jl]) cylinder(h=40,d=5);
    if (cut_nut) {
...
module cooling_fan_inlet(handle_space=0){
  cb=cooling_blower;
  // blower outlet
  bl_in_x=blower_depth(cb)-blower_base(cb)-blower_top(cb)-2*printer_off;
  bl_in_y=blower_exit(cb)-blower_wall(cb)-2*printer_off-1.3;
  bl_in_z=2;
  ddx=20-blower_lug(cb)-handle_space;
  ...
  difference(){
    union(){
      ...
      cube([20,40,2]);
      // middle
      translate_z(2)cube([blower_depth(cb),blower_exit(cb),1]);
      // inside exit
      translate([blower_base(cb)+printer_off,blower_wall(cb)+printer_off,3]) hull(){
        cube([bl_in_x,bl_in_y,eps]);
        translate_z(bl_in_z) cube([bl_in_x,bl_in_y-bl_in_z,eps]);
      }
      // handle
      translate([blower_lug(cb)+handle_space,0,1]) hull(){
        translate([0,40-8,0]) cube([ddx,8,eps]);
        translate([0,blower_screw_holes(cb)[0].x,blower_screw_holes(cb)[0].y+2]) rotate([0,90,0]) cylinder(h=ddx,d=8);
      }
    }
    // HOLES
    // screw
    translate([20+eps,blower_screw_holes(cb)[0].x,blower_screw_holes(cb)[0].y+3]){
      // M4 head
      rotate([0,-90,0]) cylinder(d1=9,d2=0,h=4.5);
      // M4 hole
      rotate([0,-90,0]) cylinder(d=4.2,h=ddx+2*eps);
    }
    // back cut
    if (handle_space>0) translate([-0.5,40.5,0]) cube([21,3,10]);
    // air out
    translate([blower_base(cb)+printer_off+print_width,blower_wall(cb)+printer_off+print_width,-eps]) cube([bl_in_x-2*print_width,bl_in_y-bl_in_z-2*print_width,6]);
  }
}
  ...
  color(pp_color) translate([0,-2,cooling_fan_z-43]) difference(){
    translate([0,-3,0]) union(){
      translate([0,0,-7-2*print_width]) {
        // right part
        cube([20,27+3,7+2*print_width]);
        // left part
        translate([-20,0,0]) cube([20,40+3,7+2*print_width]);
      }
      // to hotend
      // right outlet
    ...
    // HOLES
    // right cut
    translate([10,27+eps,-7-2*print_width-eps]) rotate([90,0,-90]) triangle(h=20+eps,a=7+2*print_width+2*eps);
    // left cut
    translate([-10,40+eps,-7-2*print_width-eps]) rotate([90,0,-90]) triangle(h=20+2*eps,a=7+2*print_width+2*eps);
    // air hole
    difference(){
      union(){
        // right main hole under fan
        translate([2,-3,-7]) cube([16,25.5+3,7]);
        // left main hole under fan
        translate([-18,-3,-7]) cube([16,38.5+3,7]);
        // right main hole to hotend
        translate([2+4,-10,-7]) cube([12,10,7-2*print_width]);
        // left main hole to hotend
        translate([-18,-10,-7]) cube([12,10.5,7-2*print_width]);
      }
      // right cut
      translate([2+8-eps,25.5,-7]) rotate([90,0,-90]) triangle(h=16+2*eps,a=7);
      // left cut
      translate([-18+8-eps,38.5,-7]) rotate([90,0,-90]) triangle(h=16+2*eps,a=7);
      // center cut
      translate([0,3,-3.5]) rotate([0,0,-90-45]) triangle(h=7,a=8.5);
      // top cut
      translate([-20,-3,-2*print_width]) cube([40,3.5,2*print_width]);
      translate([0,0.5,0]) rotate([0,90,0]) triangle(h=40,a=2*print_width);
    }
  }
  if ($preview) {
    // back M4 screw
...
module pulley_support_front_down(logo=0){
  color(pp_color) difference(){
    ...
    // front right hole
    translate([ext*4,ext/2,35-joint_extr_depth]) joint_hole(15,35,true);
    // front left hole
    translate([ext*1.5,ext/2,70-joint_extr_depth]) joint_hole(15,70,true);
    // left front hole
    translate([ext/2,ext,35-joint_extr_depth]) joint_hole(15,35,true);
    // left back hole
    translate([ext/2,ext*2,70-joint_extr_depth]) joint_hole(15,70,true);
    // M3 hole
...
module pulley_support_front_up(side=0){
  ...
  color(pp_color2) translate([0,0,beltx_shift-1]) difference(){
    ...
    translate([0,0,-beltx_shift+1]) {
      ...
      translate([ext*1.5,ext/2,70-joint_extr_depth]) joint_hole(15,70,true);
      // left front hole
      translate([ext/2,ext,35-joint_extr_depth]) joint_hole(8,35,true);
      // left back hole
      translate([ext/2,ext*2,70-joint_extr_depth]) joint_hole(15,70,true);
      // pulley hole
      translate([ext/2,ext/2,70]) joint_hole(15,50,true);
    }
...
module motor_support_x_down(){
  ...
  color(pp_color) difference(){
    ...
    // corner screw hole
    translate([ext/2,ext/2,35-joint_extr_depth]) joint_hole(10,35);
    // right screw hole
    translate([4.5*ext,ext/2,35-joint_extr_depth]) joint_hole(10,35);
    // back screw hole
    translate([ext/2,3*ext,60-joint_extr_depth]) joint_hole(10,60);
    // middle front screw hole
    translate([1.5*ext,ext/2,60-joint_extr_depth]) joint_hole(10,60);
    // motor mount holes
...
module motor_support_x_up(){
  ...
  color(pp_color2) difference(){
    translate([0,0,x_motor_z+3]) union(){
     ...
     // back screw hole
    translate([ext/2,3*ext,60-joint_extr_depth]) joint_hole(10,60,true);
     // middle front screw hole
    translate([1.5*ext,ext/2,60-joint_extr_depth]) joint_hole(10,60,true);
    // outer pulley hole
    translate([xy_o_pulley_pos.x,xy_o_pulley_pos.y-base_d+ext,60-joint_extr_depth]) joint_hole(10,60,true);
    // inner pulley hole
    translate([xy_i_pulley_pos.x,xy_i_pulley_pos.y-base_d+ext,60-joint_extr_depth]) joint_hole(10,60,true);
    // corner screw hole
    translate([ext/2,ext/2,35-joint_extr_depth]) joint_hole(22,35);
...
module motor_support_y_up(){
  ...
  color(pp_color2) difference(){
    translate([base_w,0,y_motor_z+3]) union(){
     ...
     // back screw hole
    translate([base_w-ext/2,3*ext,70-joint_extr_depth]) joint_hole(15,70,true);
     // middle front screw hole
    translate([base_w-1.5*ext,ext/2,70-joint_extr_depth]) joint_hole(15,70,true);
    // outer pulley hole
    translate([base_w-xy_o_pulley_pos.x,xy_o_pulley_pos.y-base_d+ext,75-joint_extr_depth]) joint_hole(10,60,true);
    // inner pulley hole
    translate([base_w-xy_i_pulley_pos.x,xy_i_pulley_pos.y-base_d+ext,75-joint_extr_depth]) joint_hole(10,60,true);
    // corner screw hole
    translate([base_w-ext/2,ext/2,35-joint_extr_depth]) joint_hole(37,35);
    // hole for FPC board
...
module motor_support_y_down(){
  ...
  color(pp_color) difference(){
    ...
    // corner screw hole
    translate([base_w-ext/2,ext/2,50-joint_extr_depth]) joint_hole(15,50,print_upside=true);
    // left screw hole
    translate([base_w-4.5*ext,ext/2,45-joint_extr_depth]) joint_hole(15,45,print_upside=true);
    // back screw hole
    translate([base_w-ext/2,3*ext,70-joint_extr_depth]) joint_hole(10,70,print_upside=true);
    // middle front screw hole
    translate([base_w-1.5*ext,ext/2,70-joint_extr_depth]) joint_hole(10,70);
    // motor mount holes
...
module ffc_stopper(){
  ...
  color(pp_color2) difference(){
    ...
    translate([ext/2,ext/2,joint_in_material]) joint_hole(20);
  }
  if ($preview){
...
module gantry_joint_l_cf(){
  ...
  color(pp_color) difference(){
    ...
    // bottom screw
    translate([under_screw_pos,cf_tube_size/2+cf_from_front,cf_above_carriage-1.5]) rotate([180,0,0]) joint_hole(screw_l=m5_screw5,print_upside=true);
    // front screw
...
module gantry_joint_l_cf_top(){
  ...
  color(pp_color2) difference(){
    ...
    // screw holes
    translate([px_x,px_y,m5_screw2+7]) joint_hole(10,screw_l=m5_screw2,print_upside=true);
    translate([py_x,py_y,m5_screw1+22]) joint_hole(10,screw_l=m5_screw1,print_upside=true);
    // top M5 screw
    translate([py_x+3,5,cf_above_carriage+cf_tube_size+m5_screw6+3]) joint_hole(10,screw_l=m5_screw6+5,cut_nut=false,print_upside=true);
...
module gantry_joint_r_cf_top(){
  ...
  color(pp_color2) difference(){
    ...
    translate([py_x,py_y,m5_screw1+22]) joint_hole(10,screw_l=m5_screw1,print_upside=true);
    // top M5 screw
    translate([px_x-3,5,cf_above_carriage+cf_tube_size+m5_screw6+3]) joint_hole(10,screw_l=m5_screw6+5,cut_nut=false,print_upside=true);
  }
}
...
module bb_support(rear){
  color(pp_color) difference(){
    ...
    // holes for screws
    translate([0,ext,joint_in_material]) joint_hole(0);
    translate([0,-ext,joint_in_material]) joint_hole(0);
...
module z_pulley_support(){
  ...
  color(pp_color) difference(){
    ...
    translate([ext,0,joint_in_material]) joint_hole(20);
    translate([-ext,0,joint_in_material]) joint_hole(20);
  }
}
...
module psu_mount(h_len, h_h, mirr){
  ...
  color(pp_color) difference(){
    ...
    // frame screw hole
    translate([ext/2,ext/2,joint_in_material]) joint_hole(joint_in_material);
    // psu screw hole
...
module z_endstop_mount(){
  ...
  color(pp_color) difference(){
    ...
    translate([ext/2,0.75*ext,h]) joint_hole(10,10);
  }
}
